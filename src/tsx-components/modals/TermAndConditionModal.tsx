...
import { TermAndConditionModalProps, TermAndConditionModalRef } from './modal.types'
...
const TermAndConditionModal = forwardRef<TermAndConditionModalRef, TermAndConditionModalProps>(({
    onAgree
}: TermAndConditionModalProps, ref) => {
    ...
    return (
        ...
        >
            <SafeAreaView style={{ flex: 1, backgroundColor: theme.colors.palette.green50 }}>
                {/* <View > */}
                <HeaderTitle rightIcon={SVGCross} title={modalHeaderTitle} isBack={true} onPressRight={() => setIsVisible(false)} />
                {showWebView ?
                    <WebView
                    ...
                    <ScrollView
                        onScroll={({ nativeEvent }) => {
                            ...
                            const isEndReached =
                                layoutMeasurement.height + contentOffset.y >= contentSize.height - 20; // adjust buffer
                        ...
                        {conditionArray.length > 0 && (
                            <View>
                                {conditionArray.map((item: any, index: number) => (
                                    <View key={index} style={{ marginBottom: theme.spacing.spacing.sm }}>
                                        { item?.mainHeading &&<CustomText
                                            variant="h3"
                                            style={{ marginBottom: theme.spacing.spacing.sm ,textDecorationLine:'underline'}}
                                        >
                                            {item.mainHeading}
                                        </CustomText>}
                ...
                <GenericButton
                    title="I Agree"
                    cStyle={{ marginTop: 10, marginHorizontal: 20, opacity: showButton ? 1 : 0.7 }}
                    onPress={()=>{
                        setIsVisible(false);
                        onAgree()
                    }}
