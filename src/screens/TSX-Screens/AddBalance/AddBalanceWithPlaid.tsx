...
const AddBalanceWithPlaid = () => {
  ...
  const { openPlaidLink } = usePlaidLink({
    onSuccess: () => {
      // Refresh bank accounts after successful connection
      console.log('Bank account connected successfully!');
      // You can add logic here to refresh the bank list
    },
    onCancel: () => {
  ...
  return (
    <ScreenContainer scrollable padding={0}>
      ...
      {/* Plaid Link Button - Add External Bank Account */}
      <View style={{ width: "100%", paddingHorizontal: 20, marginTop: 16 }}>
        <PlaidLinkButton
          ...
          size="medium"
          onSuccess={() => {
            console.log('External bank account added successfully!');
            // Refresh the dropdown list here
          }}
          style={{
            borderColor: theme.colors.palette.grey300,
            borderWidth: 1,
...
export default AddBalanceWithPlaid;
