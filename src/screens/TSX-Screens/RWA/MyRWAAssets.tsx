...
import { useCryptoPrices, useGetAllRWA, useGetUserHoldings } from "query/hooks";
import { defaultCrypto, defaultImage } from "utils/configs";
import { VIEW_TYPE } from "./RWA";
...
export const RenderStocks = ({ item, type }: RenderStocksComponentProps) => {
...
const RenderCrypto = ({ item }: RenderCryptoComponentProps) => {
  ...
  return (
    <View
      style={[
        customStyles(theme).sectionListRenderContainer,
        { overflow: "scroll" },
      ]}
    >
      <IconTextComponent
...
const MyRWAAssets = () => {
  ...
  // console.log("data =>", JSON.stringify(AllRWAData?.data, null, 2));
