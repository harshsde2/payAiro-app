...
import { customStyles, RenderStocks, VIEW_TYPE } from "./RWA";
...
const RealStateProfile = () => {
  ...
  const imageArray = data?.images?.filter((item: any) => item != null);

  if (data?.video != null && data?.video.includes(".mp4")) {
    if (data?.video.includes("https://app.payairo.com")) {
      imageArray.push(data?.video);
    } else {
      imageArray.push(`https://app.payairo.com${data?.video}`);
    }
  }

  // console.log("imageArray =>", JSON.stringify(data, null, 2));
  const [isModalVisible, setIsModalVissible] = useState(false);

  const isOwned = type == VIEW_TYPE.owned;

  const buttonName = isOwned ? "Sell" : "Buy";
        ...
        <View style={{ flex: 1 }}>
          ...
          <View style={[styles.mainSharesContainer]}>
            ...
            <View style={[styles.sharesContainer]}>
              ...
              <View style={[styles.sharesSection]}>
                <CustomText variant={"caption"}>Symbol</CustomText>
                <CustomText
                  fontWeight={"semiBold"}
                  color={theme.colors.palette.green700}
                  variant={"h3"}
                >
                  {data?.symbol ?? data?.asset_symbol}
                </CustomText>
              </View>
              <View style={[styles.verticalSaperator]} />
              <View style={[styles.sharesSection]}>
                <CustomText variant={"caption"}>Available Shares</CustomText>
                <CustomText
                  fontWeight={"semiBold"}
                  color={theme.colors.palette.green700}
                  variant={"h3"}
...
export default RealStateProfile;
