...
export default function AddContact() {
  ...
  const { mutate: handleAddContact, isPending, isSuccess } = useAddContact();
  ...
  const handleSave = useCallback(async () => {
    ...
    //     // Allow to auto fetch updated contact list without calling in query
    //     await queryClient.invalidateQueries(userContactKeys.recentContacts());
    //     await queryClient.refetchQueries(userContactKeys.recentContacts());
    ...
    //   } else {
    //     useDispatchAction(
    //       setErrorMsg("Wallet Address/PayAiro Tag/Email not found")
    //     );
    //   }
    // } catch (error) {
        ...
        >
          ...
          {errors.general ? (
            <View style={styles(theme).errorContainer}>
              <CustomText variant="body2" color={theme.colors.error}>
                {errors.general}
              </CustomText>
            </View>
          ) : null}
