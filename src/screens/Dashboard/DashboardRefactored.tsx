...
export default function DashboardRefactored() {
  ...
  // Redux state
  const { walletData, userData, selectedCrypto, networkLists, isCrypto } =
  ...
  const txLists = useMemo(() => {
    if (transactions.data?.data) {
      const { merchantTransactions = [], userToUserTransactions = [] } =
        transactions.data.data;
      return (
        [...merchantTransactions, ...userToUserTransactions].filter(
          (i) => i?.status === "success"
        ) || []
      );
    }
  ...
  const cryptoAssets: CryptoAsset[] = useMemo(() => {
    if (cryptoBalance?.data?.data?.data) {
      return (
        cryptoBalance.data.data.data.filter(
          (asset: any) => asset?.assetType !== "usd"
        ) || []
      );
    }
    return [];
  ...
  const totalDisbursable = useMemo(() => {
    if (cryptoAssets.length > 0) {
      return Number(
        cryptoAssets.reduce(
          (sum: number, asset: CryptoAsset) => sum + (asset?.disbursable || 0),
          0
        )
      );
    }
  ...
  const totalDisbursablePending = useMemo(() => {
    if (cryptoAssets.length > 0) {
      return Number(
        cryptoAssets.reduce(
          (sum: number, asset: CryptoAsset) => sum + (asset?.pending || 0),
          0
        )
      );
    }
              ...
              >
                ...
                >
                  ...
                  >
                    <SvgXml
                      xml={SVGUSD}
                      width={isCrypto ? undefined : 30}
                      height={isCrypto ? undefined : 30}
                    />
                    <View
                      style={{
                        flexDirection: "row",
